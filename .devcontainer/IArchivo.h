...
private:
    Seleccion* buscarSeleccionEnPool(const std::string& nombrePais, Seleccion* poolSelecciones, int cantSelecciones) {
        for (int i = 0; i < cantSelecciones; i++) {
            if (poolSelecciones[i].getNombrePais() == nombrePais) return &poolSelecciones[i];
        }
        return nullptr;
    }
...
public:
    ...
    void cargarGrupo(const std::string& nombreArchivo, Grupo& grupoDestino, Seleccion* poolSelecciones, int& indiceGlobalSelecciones) override {
        ...
        if (!archivo.is_open()) return;
        std::string nombreGrupo; int cantSelecciones = 0;
        std::getline(archivo, nombreGrupo); archivo >> cantSelecciones; archivo.ignore();

        for (int i = 0; i < cantSelecciones; i++) {
            std::string nombrePais; std::getline(archivo, nombrePais);
            int cantJugadores = 0; archivo >> cantJugadores; archivo.ignore();
    ...
    void cargarPartidos(const std::string& nombreArchivo, Partido* poolPartidos, int& cantPartidosDestino, Seleccion* poolSelecciones, int cantSelecciones, Grupo& grupoDestino) override {
        std::ifstream archivo(nombreArchivo);
        if (!archivo.is_open()) return;
        int cantPartidosGuardados = 0; archivo >> cantPartidosGuardados; archivo.ignore();

        for (int i = 0; i < cantPartidosGuardados; i++) {
            std::string nombreLocal, nombreVisitante; int golesLocal = 0, golesVisitante = 0;
            std::getline(archivo, nombreLocal); std::getline(archivo, nombreVisitante);
            archivo >> golesLocal; archivo >> golesVisitante; archivo.ignore();
